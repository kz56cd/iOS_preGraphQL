...
public final class AllPostsQuery: GraphQLQuery {
  ...
  public struct Data: GraphQLSelectionSet {
    ...
    public init(posts: [Post?]? = nil) {
      self.init(snapshot: ["__typename": "Query", "posts": posts.flatMap { (value: [Post?]) -> [Snapshot?] in value.map { (value: Post?) -> Snapshot? in value.flatMap { (value: Post) -> Snapshot in value.snapshot } } }])
    }

    public var posts: [Post?]? {
      get {
        return (snapshot["posts"] as? [Snapshot?]).flatMap { (value: [Snapshot?]) -> [Post?] in value.map { (value: Snapshot?) -> Post? in value.flatMap { (value: Snapshot) -> Post in Post(snapshot: value) } } }
      }
      set {
        snapshot.updateValue(newValue.flatMap { (value: [Post?]) -> [Snapshot?] in value.map { (value: Post?) -> Snapshot? in value.flatMap { (value: Post) -> Snapshot in value.snapshot } } }, forKey: "posts")
      }
    }
